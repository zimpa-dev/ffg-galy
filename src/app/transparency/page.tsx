  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <p className="text-muted-foreground animate-pulse font-medium">Chargement de la galerie...</p>
      </div>
    );
  }
    ...
    <div className="container mx-auto px-4 py-16 md:py-24 space-y-12 animate-in fade-in duration-700">
      ...
      <div className="text-center space-y-6 max-w-3xl mx-auto">
        ...
        <p className="text-xl text-muted-foreground leading-relaxed">
          Découvrez en images la réalité de nos actions sur le terrain. Chaque photo témoigne de la solidarité et du changement que nous construisons ensemble.
        </p>
      </div>

      {/* Controls */}
      <div className="flex flex-col md:flex-row gap-6 items-center justify-between bg-zinc-50 dark:bg-zinc-900 p-6 rounded-[2.5rem] border shadow-sm">
          ...
          {galleryCategories.map((cat) => (
            <Button
              ...
              className={`rounded-full px-6 transition-all ${
                activeCategory === cat.id ? "shadow-lg scale-105" : "text-muted-foreground"
              }`}
            >
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            className="w-full bg-white dark:bg-zinc-800 rounded-full py-2.5 pl-12 pr-6 border-none focus:ring-2 focus:ring-primary/20 text-sm transition-all"
            value={searchQuery}
      ...
      {filteredItems.length > 0 ? (
          ...
          {filteredItems.map((item) => (
            <Dialog key={item.id}>
              <DialogTrigger asChild>
                <div className="group cursor-pointer relative aspect-[4/5] rounded-[2.5rem] overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-500">
                  <Image
                    src={item.image}
                    alt={item.title}
                    fill
                  ...
                  <div className="absolute inset-0 p-8 flex flex-col justify-end text-white translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                    <div className="space-y-2">
                      <Badge className="bg-white/20 backdrop-blur-md border-none text-white font-bold uppercase tracking-wider text-[10px]">
              ...
              <DialogContent className="max-w-4xl rounded-[3rem] p-0 overflow-hidden border-none bg-zinc-950 text-white">
                <div className="grid grid-cols-1 md:grid-cols-2">
                  ...
                  <div className="p-8 md:p-12 space-y-8 flex flex-col justify-center">
                    <div className="space-y-4">
                      <Badge className="bg-primary/20 text-primary border-none text-xs font-bold uppercase px-3 py-1 rounded-full">
                        {galleryCategories.find(c => c.id === item.category)?.label}
                      </Badge>
                      <DialogTitle className="text-4xl font-headline font-bold">{item.title}</DialogTitle>
                    </div>
                    ...
                    <div className="pt-8 border-t border-white/10">
                      <Button className="w-full bg-white text-black hover:bg-zinc-200 rounded-full h-12 font-bold" onClick={() => (window.location.href = '/donate')}>
                        Soutenir ce type d'action
                      </Button>
                    </div>
                  </div>
                </div>
        ...
        <div className="py-32 text-center bg-zinc-50 dark:bg-zinc-900 rounded-[3rem] border border-dashed">
          ...
          <Button variant="link" className="mt-4" onClick={() => {setActiveCategory("all"); setSearchQuery("");}}>
      ...
      {/* Call to Action */}
      <section className="bg-primary p-12 md:p-20 rounded-[3rem] text-white overflow-hidden relative group">
        <div className="relative z-10 text-center space-y-8">
          <h2 className="text-3xl md:text-5xl font-headline font-bold max-w-2xl mx-auto">
            Aidez-nous à capturer de nouveaux moments de solidarité.
          </h2>
          <div className="flex flex-wrap justify-center gap-4">
            <Button size="lg" className="bg-white text-primary hover:bg-zinc-100 rounded-full px-10 h-14 font-bold text-lg" onClick={() => (window.location.href = '/donate')}>
              Faire un don
            </Button>
            <Button size="lg" variant="outline" className="border-white text-white hover:bg-white/10 rounded-full px-10 h-14 font-bold text-lg" onClick={() => (window.location.href = '/volunteer')}>
              Devenir bénévole
            </Button>
          </div>
        </div>
        <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 -mr-32 -mt-32 rounded-full blur-3xl group-hover:scale-110 transition-transform duration-700" />
        <div className="absolute bottom-0 left-0 w-64 h-64 bg-secondary/20 -ml-32 -mb-32 rounded-full blur-3xl" />
      </section>
    </div>
  );
}
